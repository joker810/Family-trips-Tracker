...
async function getCurrentUser(){
  ...
  users=result.rows;
  return users.find(obj=>obj.id==currentUserId);
}
...
app.post("/new", async (req, res) => {
 ...
 const color=req.body.color;
  ...
  const use= await db.query("INSERT INTO users(name,color) VALUES ($1,$2) RETURNING *",[name,color]);
  
  const user=use.rows[0].id;
  currentUserId=user;
